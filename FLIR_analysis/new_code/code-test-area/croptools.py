...
class SelectUtils(object):
    ...
    def write(self,fname,volt,curr):
        ...
        for i, label in enumerate(self.saved_labels):
            output.write("%s,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n" % (label,self.saved_max[i],self.saved_maxavg[i],self.saved_min[i],self.saved_avg[i],volt,curr,volt*curr,self.saved_fractions[i]))
        output.close()
...
def manualcrop(filelist, dirname):
    ...
    for file in filelist:
        ...
        if selector != 0 and input("Use same regions as previous image? [y/n] : ") == "y":
            selector = SelectUtils(ax,grid,csv,regions=selector.saved_regions,labels=selector.saved_labels,fracs=selector.saved_fractions)
