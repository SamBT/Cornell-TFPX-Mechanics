...
def tempvis(csv_file, tmax, tmin, cmap, showortell='show'):
    ...
    if (input("Use same regions as last image?[y/n]: ") == "y"):
        ...
        print("--------------------------------------")
        return (saved_localmax, saved_localmaxavg, saved_localmin, saved_localavg, saved_labels, volt, curr)
    else:
        cached_paths = []
        cached_labels = []
...
def manualcrop(csv_file, dirname):
    ...
    (max, maxavg, min, avg, labels, volt, curr) = tempvis(csv_file, tmax, tmin, cmap='default', showortell='show')
    ...
    for i, label in enumerate(labels):
        output.write("%s,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n" % (label,max[i],maxavg[i],min[i],avg[i],volt,curr,volt*curr))
    output.close()
