...
def tempvis(csv_file, tmax, tmin, cmap, dirname):
    ...
    title ='\n Heat map of ' + csv_file[:-4] + '\n'
    fig, ax = plt.subplots()
    if cmap=='default':
        ax.matshow(csv, interpolation='none', vmin=tmin, vmax=tmax)
    else:
        ax.matshow(csv, interpolation='none', vmin=tmin, vmax=tmax, cmap=cmap)
    ax.set(xlabel='\ndistance in pixels',ylabel='distance in pixels',title=title)
    saveas = dirname+'/'+csv_file[:-4]+'.pdf'
