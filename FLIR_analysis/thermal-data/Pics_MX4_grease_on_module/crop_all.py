...
def tempvis(csv_file, tmax, tmin, cmap, showortell='show'):
    ...
    def onselect(verts):
        ...
        if (save == "y"):
            saved_localmax.append(np.amax(data))
            saved_localmin.append(np.amin(data))
            saved_localavg.append(np.average(data))
    ...
    print("--------------------------------------")
    return (saved_localmax, saved_localmin, saved_localavg, saved_labels, saved_volts, saved_currs)
...
def manualcrop(csv_file, dirname):
    ...
    (max, min, avg, labels, volts, currs) = tempvis(csv_file, tmax, tmin, cmap='default', showortell='show')
    filename = dirname+'/'+'cropped_'+csv_file.split("/")[-1]
    output = open(filename,"a")
    if os.stat(filename).st_size == 0:
        output.write("Region,Max T,Min T, Avg T, Voltage, Current, Power\n")
    for i, label in enumerate(labels):
        output.write("%s,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n" % (label,max[i],min[i],avg[i],volts[i],currs[i],volts[i]*currs[i]))
    output.close()
